...
public class AlarmReportController extends AbstractController implements InitializingBean {
     ...
     * Logging
     */
    private static final Logger LOG = LoggerFactory.getLogger(AlarmReportController.class);
    ...
    protected ModelAndView handleRequestInternal(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
		...
		try {
    		...
    		ex.printStackTrace();
    		LOG.error("Unable to marshall your alarm bean class because of  "+ex.getMessage());
    	} finally {
    		if(out != null){
    			out.close();
    		}
        ...
        try{	
    		if(addressForServer!=null && addressForServer!=" "){
    			...
    			LOG.info("Client Response is " + clientResponse.getStatus());
    		} else {
    			LOG.error("Unable to call junose rest api because junos server address ["+addressForServer+"] is invalid");
    		}
    	} catch(Exception ex) {
    		ex.printStackTrace();
    		LOG.error("Unable to call junose rest api from alarm report controller because of "+ex.getMessage());
    	}
        ...
        try {
        	...
        	LOG.error("Could not get the task response from junose rest api");
        }
        ...
        LOG.info("Job Id["+taskId+"] for your AlarmReportController action");
