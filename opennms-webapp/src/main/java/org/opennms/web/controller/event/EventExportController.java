...
public class EventExportController extends AbstractController implements
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		LOG.info("Enter into the EventExportController action");
		...
		try {
			...
			ex.printStackTrace();
			LOG.error("Unable to marshall your event bean class because of  "
					+ ex.getMessage());
		} finally {
			if (out != null) {
				out.close();
		...
		try {
			if (addressForServer != null && addressForServer != " ") {
				...
				LOG.error("Unable to call junose rest api because junos server address ["
						+ addressForServer + "] is invalid");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			LOG.error("Unable to call junose rest api from alarm report controller because of "
					+ ex.getMessage());
		}
		...
		LOG.info("Terminated from the EventExportController action");
